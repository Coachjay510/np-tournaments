...
export default function Teams() {
  ...
  async function handleBulkAssignOrg() {
    ...
    setBulkSaving(true)
    const masterIds = [...selectedIds].map(id => {
      const team = pagedTeams.find(t => t.id === id)
      return team?.master_team_id
    }).filter(Boolean)
    const uniqueMasterIds = [...new Set(masterIds)]
    await Promise.all(uniqueMasterIds.map(masterId =>
      supabase.from('bt_master_teams').update({ organization_id: Number(bulkOrg) }).eq('id', masterId)
    ))
    setBulkSaving(false)
    setSelectedIds(new Set())
    setBulkOrg('')
  ...
  const AGE_MAP = {
    '6u': '7u', '7u': '8u', '8u': '9u', '9u': '10u',
    '10u': '11u', '11u': '12u', '12u': '13u', '13u': '14u',
    '14u': '15u', '15u': '16u', '16u': '17u', '17u': '18u', '18u': '18u',
  }

  const DIV_MAP = {
    '6u_boys': '7u_boys', '7u_boys': '8u_boys', '8u_boys': '9u_boys',
    '9u_boys': '10u_boys', '10u_boys': '11u_boys', '11u_boys': '12u_boys',
    '12u_boys': '13u_boys', '13u_boys': '14u_boys', '14u_boys': '15u_boys',
    '15u_boys': '16u_boys', '16u_boys': '17u_boys', '17u_boys': '18u_boys',
    '6u_girls': '7u_girls', '7u_girls': '8u_girls', '8u_girls': '9u_girls',
    '9u_girls': '10u_girls', '10u_girls': '11u_girls', '11u_girls': '12u_girls',
    '12u_girls': '13u_girls', '13u_girls': '14u_girls', '14u_girls': '15u_girls',
    '15u_girls': '16u_girls', '16u_girls': '17u_girls', '17u_girls': '18u_girls',
    '10_11u_boys': '11_12u_boys', '11_12u_boys': '12_13u_boys',
  }

  async function previewAgeUp() {
    const { data } = await supabase.from('bt_master_teams').select('id, display_name, age_group, ranking_division_key').not('age_group', 'is', null)
    const preview = (data || []).map(t => ({
      id: t.id,
      name: t.display_name,
      oldAge: t.age_group,
      newAge: AGE_MAP[t.age_group] || t.age_group,
      oldDiv: t.ranking_division_key,
      newDiv: DIV_MAP[t.ranking_division_key] || t.ranking_division_key,
      changed: AGE_MAP[t.age_group] !== t.age_group,
    })).filter(t => t.changed)
  ...
  async function handleAgeUp() {
    if (!window.confirm('This will age up ALL teams by 1 year. This cannot be undone. Continue?')) return
    setAgingUp(true)
    await Promise.all(ageUpPreview.map(t =>
      supabase.from('bt_master_teams').update({
        age_group: t.newAge,
        ranking_division_key: t.newDiv,
      }).eq('id', t.id)
    ))
    setAgingUp(false)
    setShowAgeUp(false)
    refresh()
    alert('Age-up complete! All teams have been updated.')
  }
